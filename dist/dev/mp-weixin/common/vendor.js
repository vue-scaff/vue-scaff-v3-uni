"use strict";
var scaff = require("../scaff.js");
var common_assets = require("./assets.js");
var custom_plus = require("../custom/plus.js");
var directives_move = require("../directives/move.js");
var filters_cash = require("../filters/cash.js");
var i18n_enUS = require("../i18n/en-US.js");
var i18n_zhCN = require("../i18n/zh-CN.js");
var registry_api = require("../registry/api.js");
var registry_host = require("../registry/host.js");
var registry_mixin = require("../registry/mixin.js");
var registry_route = require("../registry/route.js");
var registry_store = require("../registry/store.js");
var tools_hello = require("../tools/hello.js");
var utils_empty = require("../utils/empty.js");
var utils_foreach = require("../utils/foreach.js");
var utils_uri = require("../utils/uri.js");
var pages_start_store = require("../pages/start/store.js");
...
function dynamicSlotName(name2) {
  return name2 === "default" ? SLOT_DEFAULT_NAME : name2;
}
const customizeRE = /:/g;
  ...
  if (interceptor2 && Object.keys(interceptor2).length) {
    if (isArray$1(interceptor2.invoke)) {
      ...
      return res.then((options2) => {
        return api(wrapperOptions(interceptor2, options2), ...params);
      });
    } else {
...
function mergeInterceptorHook(interceptors2, interceptor2) {
  Object.keys(interceptor2).forEach((hook) => {
    if (isFunction$1(interceptor2[hook])) {
...
function removeInterceptorHook(interceptors2, interceptor2) {
  ...
  Object.keys(interceptor2).forEach((name2) => {
    const hooks = interceptors2[name2];
    const hook = interceptor2[name2];
...
const addInterceptor = defineSyncApi(API_ADD_INTERCEPTOR, (method, interceptor2) => {
  if (isString$1(method) && isPlainObject$1(interceptor2)) {
...
function populateParameters(fromRes, toRes) {
  ...
  const parameters = {
    appId: "__UNI__B522EC0",
    appName: "vue-scaff-uni",
    appVersion: "1.0.0",
    appVersionCode: "100",
...
function doWatch(source, cb, { immediate, deep: deep2, flush, onTrack, onTrigger } = EMPTY_OBJ) {
  if (!cb) {
    if (immediate !== void 0) {
  ...
  const job = () => {
    ...
    if (cb) {
      ...
      if (deep2 || forceTrigger || (isMultiSource ? newValue.some((v, i) => hasChanged(v, oldValue[i])) : hasChanged(newValue, oldValue)) || false) {
        if (cleanup) {
          cleanup();
        }
...
function resolve(registry2, name2) {
...
const warnRuntimeUsage = (method) => warn$1$1(`${method}() is a compiler-hint helper that is only usable inside <script setup> of a single file component. Its arguments should be compiled away and passing it at runtime has no effect.`);
  ...
  if (type === "object" && src !== null) {
    ...
    if (isArray$1(src)) {
      ...
      for (const name2 in src) {
        if (hasOwn$2(src, name2)) {
          copy2[name2] = clone(src[name2], seen);
        }
      }
    }
    return copy2;
...
function useCssModule(name2 = "$style") {
  {
    ...
    const mod = modules2[name2];
    if (!mod) {
      warn$1$1(`Current instance does not have CSS module named "${name2}".`);
      return EMPTY_OBJ;
    }
    return mod;
  }
...
function dynamicSlot(names) {
  ...
  return names.map((name2) => dynamicSlotName(name2));
}
...
const r = (name2, props, key) => renderSlot(name2, props, key);
...
function callHook(name2, args) {
  if (name2 === "mounted") {
    ...
    this.$.isMounted = true;
    name2 = "m";
  } else if (name2 === "onLoad" && args && args.__id__) {
...
function findHooks(vueOptions, hooks = /* @__PURE__ */ new Set()) {
  if (vueOptions) {
    Object.keys(vueOptions).forEach((name2) => {
      if (name2.indexOf("on") === 0 && isFunction$1(vueOptions[name2])) {
        hooks.add(name2);
      }
    });
    {
...
var promise = ((proxy) => class Presence {
  constructor(actuator) {
    proxy = this, actuator(this.resolve, this.reject);
    return proxy;
  }
  resolve(result) {
    proxy.result = result;
  ...
  then(fn) {
    proxy.result = fn(proxy.result, proxy.sneak);
    return proxy;
  }
  catch(fn) {
    if (proxy.error !== void 0) {
      fn(proxy.error);
    }
    return proxy;
  }
...
const foreach$2 = (source = {}, callback = noop$2) => {
...
const compare = (origin, target) => {
  return origin === void 0 ? target : origin;
};
const check$1 = (value, mode = Object) => {
...
const deepeach = (source = {}, callback = noop$2, clone2 = false, root = []) => {
  if (clone2) {
    source = copy$1(source);
  }
  source = foreach$2(source, (value, key, parent) => {
    const path = [...root, key];
    return check$1(value) ? deepeach(value, callback, clone2, path) : callback(value, key, path, parent);
  });
  return source;
};
...
function foreach$1(source, callback, { clone: clone2 = false, deep: deep2 = false, stop: stop2 = false, step = 1, long = 0 } = {}) {
  let json = clone2 === true ? copy(source) : source;
  if (deep2 === true) {
  ...
  if (json.length !== void 0) {
    for (let index2 = 0; index2 < json.length; index2++) {
      ...
      if (conclusion(index2, json, callback) === stop2) {
        break;
      }
    }
    return json;
  ...
  for (let key in json) {
    ...
    if (conclusion(key, json, callback) === stop2) {
      break;
    }
  }
  return json;
}
var matcher = (path, exp) => {
  if (exp) {
    const matcher2 = exp.exec(path);
    let raggae = "";
    foreach(matcher2, (item, index2) => {
      if (index2) {
        raggae += `${raggae ? "-" : ""}${matcher2[index2].replace(/\//g, "-")}`;
      }
    });
    if (raggae) {
      return raggae;
...
var def$1 = (dependency) => dependency.default || dependency;
var gatherer = (source, callback) => {
  const result = {};
...
var console$1 = (options = {}) => {
  ...
  const bind = (name2, label, color) => ({ [name2]: console.log.bind(console, `%c${label}`, css(color)) });
  const target = {};
  foreach(preset, ({ label, color }, name2) => Object.assign(target, bind(name2, label, color)));
  return target;
};
function empty(target) {
  return [void 0, null].includes(target);
}
var hand = (target, params, defers) => {
  const is2 = typeof target === "function";
  defers = empty(defers) ? params : defers;
  const result = is2 ? target(params) : empty(target) ? defers : target;
  return empty(result) ? defers : result;
};
var is = (target, type) => {
  if ([void 0, "undefined"].includes(target)) {
    return false;
  }
  return target.constructor === type;
};
function merge$1(origin, target) {
  for (let key in target) {
    origin[key] = origin[key] && origin[key].constructor === Object ? merge$1(origin[key], target[key]) : origin[key] = target[key];
  }
  return origin;
}
var suffix = (template) => {
  const matcher2 = template.match(/\.(\w+)$/);
  return matcher2 ? matcher2[1] : false;
};
function lexer(str) {
  var tokens = [];
  var i = 0;
...
function tokensToRegexp(tokens, keys, options) {
  ...
  for (var _i = 0, tokens_1 = tokens; _i < tokens_1.length; _i++) {
    ...
    if (typeof token === "string") {
      ...
      var suffix2 = escapeString(encode2(token.suffix));
      if (token.pattern) {
        if (keys)
          keys.push(token);
        if (prefix || suffix2) {
          if (token.modifier === "+" || token.modifier === "*") {
            ...
            route += "(?:".concat(prefix, "((?:").concat(token.pattern, ")(?:").concat(suffix2).concat(prefix, "(?:").concat(token.pattern, "))*)").concat(suffix2, ")").concat(mod);
          } else {
            route += "(?:".concat(prefix, "(").concat(token.pattern, ")").concat(suffix2, ")").concat(token.modifier);
          }
        } else {
          if (token.modifier === "+" || token.modifier === "*") {
        ...
        route += "(?:".concat(prefix).concat(suffix2, ")").concat(token.modifier);
      }
    }
  }
  if (end) {
...
var translate$1 = (template) => {
  template = template.replace(/\*{1}\./g, ":file.");
  template = template.replace(/\*{2}/g, "(.*)");
  return pathToRegexp(template);
};
var replace = (uri, exp = new RegExp(/\{\S+\}/g)) => {
  if (exp.test(uri)) {
    uri.replace(exp, (word) => {
    });
  }
  return exp.test(uri) ? (j2, o2, e2, n2, i, x, a = 0, cache2 = [j2, o2, e2, n2, i, x]) => uri.replace(exp, (word) => cache2[a++]) : uri;
};
var bless = (registry2) => {
  if (is(registry2.api, Function)) {
    registry2.api = registry2.api(registry2.host || {}, replace);
  }
  return registry2;
};
...
const validateAndMerge = (...sources) => {
  for (const source of sources) {
    if ((!isObject$3(source) || Array.isArray(source)) && typeof source !== "undefined") {
...
const timeout = async (request, abortController, options) => new Promise((resolve2, reject) => {
  ...
  void options.fetch(request).then(resolve2).catch(reject).then(() => {
...
var ky$1 = ky;
const METHODS = `get post put head patch delete`.split(" ");
const noop$1 = async () => {
};
const interceptor = {
  request: noop$1,
  ...
  retry: noop$1,
  error: noop$1,
  bodyParse: (params) => params,
  resultParse: (params) => params
};
function json2query(json, query = ``) {
  Object.keys(json).forEach((key) => {
    query += `${query.length ? "&" : "?"}${key}=${json[key]}`;
  });
  return query;
}
class HTTP {
  ...
  init(modules2 = {}) {
    const scope = (api) => {
      METHODS.forEach((method) => {
        modules2[method] = async (params, options) => await this.send(method, api, params, options);
      });
      return modules2;
    };
    return scope.interceptor = interceptor, scope;
  }
  pack(method, api, params) {
    const data = {};
    [`get`, `head`].includes(method) ? api += json2query(params) : data.body = interceptor.bodyParse(params);
    return { link: api, data };
  }
  async send(method = `get`, api = ``, params = {}, options = {}) {
    const hooks = {
      beforeRequest: [async (request) => await interceptor.request(request)],
      afterResponse: [async (request, options2, response) => await interceptor.response(response, request)],
      beforeError: [async (error) => await interceptor.error(error)],
      beforeRetry: [async (cross) => await interceptor.retry(cross)]
    };
    const { link, data } = this.pack(method, api, params);
    const http2 = ky$1.extend({ hooks });
    const result = http2[method](link, { method, hooks, ...data, ...options });
...
const methods = `get post put delete connect head options trace`.split(" ");
let uni = uniTolerance();
function http(url = "", headers = {}) {
  const handlers = {};
  methods.map((method) => {
    handlers[method] = (params) => {
      const { data, header } = http.interceptor.request(params, headers);
      data = merge$1(data, params);
      header = merge$1(header, headers);
      return new Promise((resolve2, reject) => {
        uni.request({
          url,
          header,
          data,
          success: (response) => resolve2(http.interceptor.response(response)),
          fail: (error) => reject(http.interceptor.error(error))
        });
      });
    };
  });
...
var gemini = (app, { registry: registry2, resource: resource2 }) => {
  ...
  if (resource2.util) {
    resource2.util = { ...resource2.util, $http, $api };
  }
  resource2.$http = $http;
  return { resource: resource2, registry: registry2 };
};
var extensor = (source = {}, handler = () => {
}, special) => {
  const express = {
    configure: null,
    instanter: null,
...
var mapState = normalizeNamespace(function(namespace, states2) {
  var res = {};
  if (!isValidMap(states2)) {
    console.error("[vuex] mapState: mapper parameter must be either an Array or an Object");
  }
  normalizeMap(states2).forEach(function(ref2) {
    var key = ref2.key;
    var val = ref2.val;
...
function check(target, type = Object) {
  return [null, void 0].includes(target) ? false : target.constructor === type;
}
...
var merge = (origin = {}, target) => deep(origin, target);
var noop = new Function();
function transfer(value, custom = {}) {
  if ([void 0, null, "undefined", "null"].includes(value)) {
    return "";
  }
...
var reset = (target, custom) => {
  return deepeach(target, (value) => {
    return transfer(value, custom);
  });
};
var catcher$1 = async (handler, root, params) => {
  let result = {};
  try {
...
function states$1(settings = {}) {
  return {
    error: false,
    ...settings
  };
}
function actions$1(settings = {}) {
  const json = {
    async STATE_UPDATE({ commit: commit2 }, data) {
      return await commit2("STATE_UPDATE", data);
    },
    async STATE_CLEAR({ commit: commit2, state }, namespace) {
      return await commit2("STATE_UPDATE", reset(state[namespace]));
    }
  };
  foreach$1(settings, (action, name2) => {
    json[name2] = async (root, params) => {
      const result = await catcher$1(action, root, params);
...
function mutations$1(settings = {}) {
  const json = {
    STATE_UPDATE(state, data) {
      return merge(state, data);
    }
  };
  foreach$1(settings, (mutation, name2) => {
    json[name2] = (state, data) => mutation(state, data);
  });
  return json;
}
...
const { states, mutations, actions } = store$1;
var fast = (modules2) => {
  return foreach$1(modules2, (module) => {
...
var store = (app, { util: util2, store: store2 }, registry2) => {
  let modules2 = foreach(store2, (hook, key) => {
    return {
      key: cross2camel(key),
      value: hook(util2, registry2.api)
    };
  });
  return extensor((config) => {
    const configure = hand(config, modules2, {});
    modules2 = fast(modules2);
    return createStore({
      ...registry2.store(util2),
      modules: modules2,
      ...configure
    });
  }, (omni) => {
    omni.update = (namespace, data = {}) => omni.dispatch(`${namespace}/STATE_UPDATE`, data);
    app.use(omni);
  }, modules2);
};
var util = (app, { util: util2 }, registry2) => {
  return extensor(util2, (omni) => {
    app.config.globalProperties.$util = foreach(omni, (value, key) => {
      return { key: cross2camel(key), value };
    });
  });
};
var filter = (app, { filter: filter2 }, registry2) => {
  return extensor(filter2, (omni) => {
    app.config.globalProperties.$filter = omni;
  });
};
var directive = (app, { directive: directive2 }, registry2) => {
  return extensor(directive2, (omni) => {
    foreach(omni, (command, key) => app.directive(key, command));
  });
};
var component = (app, { util: util2, component: component2 }, registry2) => {
  return extensor(foreach(component2, (depend, key) => ({ key: depend.name || key.replace(/\-index$/, ""), value: depend })), (omni) => {
    app.config.globalProperties.$component = foreach(omni, (depend, key) => {
      app.component(camel2cross(key), depend);
    });
  });
};
var style = (app, { style: style2 }, registry2) => {
  return extensor(style2, (omni) => {
    app.config.globalProperties.$style = omni;
  });
};
/*!
...
function handleMissing(context2, key, locale, missingWarn, type) {
  const { missing, onWarn } = context2;
  {
    const emitter2 = context2.__v_emitter;
    if (emitter2) {
  ...
  if (missing !== null) {
    const ret = missing(context2, locale, key, type);
...
function translate(context2, ...args) {
  const { fallbackFormat, postTranslation, unresolving, fallbackLocale, messages } = context2;
  ...
  const locale = isString(options.locale) ? options.locale : context2.locale;
  escapeParameter && escapeParams(options);
  let [format2, targetLocale, message] = !resolvedMessage ? resolveMessageFormat(context2, key, locale, fallbackLocale, fallbackWarn, missingWarn) : [
    key,
    locale,
    messages[locale] || {}
  ];
  ...
  if (isString(format2) && context2.messageCompiler == null) {
  ...
  const msg = !isMessageFunction(format2) ? compileMessageFormat(context2, key, targetLocale, format2, cacheBaseKey, errorDetector) : format2;
  if (occurred) {
    return format2;
  }
...
function resolveMessageFormat(context2, key, locale, fallbackLocale, fallbackWarn, missingWarn) {
  const { messages, onWarn } = context2;
  ...
  for (let i = 0; i < locales.length; i++) {
    ...
    if (isString(format2) || isFunction(format2))
      break;
    const missingRet = handleMissing(context2, key, targetLocale, missingWarn, type);
    if (missingRet !== key) {
      format2 = missingRet;
    }
    from = to;
...
function compileMessageFormat(context2, key, targetLocale, format2, cacheBaseKey, errorDetector) {
...
function getCompileOptions(context2, locale, key, source, warnHtmlMessage, errorDetector) {
  return {
    warnHtmlMessage,
    onError: (err) => {
...
function getMessageContextOptions(context2, locale, message, options) {
  ...
  const resolveMessage = (key) => {
    ...
    if (isString(val)) {
      ...
      const msg = compileMessageFormat(context2, key, locale, val, key, errorDetector);
...
function createComposer(options = {}) {
  ...
  function t2(...args) {
    return wrapWithDeps((context2) => translate(context2, ...args), () => parseTranslateArgs(...args), "translate", (root) => root.t(...args), (key) => key, (val) => isString(val));
  }
  function rt(...args) {
    const [arg1, arg2, arg3] = args;
  ...
  function d2(...args) {
    return wrapWithDeps((context2) => datetime(context2, ...args), () => parseDateTimeArgs(...args), "datetime format", (root) => root.d(...args), () => MISSING_RESOLVE_VALUE, (val) => isString(val));
  }
  function n2(...args) {
    return wrapWithDeps((context2) => number(context2, ...args), () => parseNumberArgs(...args), "number format", (root) => root.n(...args), () => MISSING_RESOLVE_VALUE, (val) => isString(val));
  }
  function normalize(values) {
    return values.map((val) => isString(val) ? createVNode$1() : val);
  }
  ...
  function transrateVNode(...args) {
    return wrapWithDeps((context2) => {
      ...
      return ret;
    }, () => parseTranslateArgs(...args), "translate", (root) => root[TransrateVNodeSymbol](...args), (key) => [createVNode$1()], (val) => isArray(val));
  }
  function numberParts(...args) {
    return wrapWithDeps((context2) => number(context2, ...args), () => parseNumberArgs(...args), "number format", (root) => root[NumberPartsSymbol](...args), () => [], (val) => isString(val) || isArray(val));
  }
  function datetimeParts(...args) {
    return wrapWithDeps((context2) => datetime(context2, ...args), () => parseDateTimeArgs(...args), "datetime format", (root) => root[DatetimePartsSymbol](...args), () => [], (val) => isString(val) || isArray(val));
  }
...
({
  ...
  setup(props, context2) {
    ...
    return renderFormatter(props, context2, NUMBER_FORMAT_KEYS, (...args) => i18n[NumberPartsSymbol](...args));
  }
});
...
({
  ...
  setup(props, context2) {
    const i18n = props.i18n || useI18n({ useScope: "parent", __useComponent: true });
    return renderFormatter(props, context2, DATETIME_FORMAT_KEYS, (...args) => i18n[DatetimePartsSymbol](...args));
  }
});
...
var md = (app, { util: util2, md: md2 }, registry2) => {
  return extensor(md2, (omni) => {
    app.config.globalProperties.$md = foreach(omni, (depend, key) => {
      app.component(`md-${camel2cross(key)}`, depend);
    });
  });
};
const modules = { store, util, filter, directive, component, style, md };
var index = {
  ...modules
};
var worker = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  "default": index
}, Symbol.toStringTag, { value: "Module" }));
var ext = ({ resource: resource2, registry: registry2 }, app, extension, runtime, next) => {
  const packages = foreach(resource2, (origin, key) => {
    const walker = def$1(worker);
    const value = key in walker ? walker[key](app, resource2, registry2) : app.config.globalProperties[key] = origin;
    return { key, value };
  });
  runtime({ ...packages }, () => {
    foreach(packages, (pkg, key) => pkg.install ? extension(pkg, key) : pkg);
    next();
  });
};
var init = (app, context2) => {
  global.$console = console$1();
  app.mixin(context2.registry.mixin);
  return gemini(app, context2);
};
const _sfc_main$1 = {
  onLaunch: function() {
...
var App$1 = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__file", "/Users/joenix/Github/@scaff-v3-template/vue-scaff-v3-uni/src/App.vue"]]);
var __glob_2_0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  "default": App$1
}, Symbol.toStringTag, { value: "Module" }));
global.Vue = Vue;
var applet = createApp$2(App$1);
var Scaff$1 = (context2) => {
  return class Scaff {
    constructor(runtime = () => {
    }, app = applet) {
      global.App = app;
      ext(init(app, context2), app, (part) => app.use(part), (packages, next) => {
        runtime({ ...packages, app }, next);
      }, () => app.mount({ "conf": { "main": { "app": "App.vue", "mount": "#app" }, "mock": false, "apollo": false, "registry": { "host": true, "api": true, "route": true, "store": true, "mixin": true }, "extract": { "util": "/utils/*.js", "filter": "/filters/*.js", "directive": "/directives/*.js", "route": "/pages/**/route.js", "store": "/pages/**/store.js", "component": "/components/**/*.vue", "style": "/styles/*.less", "i18n": "/i18n/*.js", "md": "/pages/**/*.md", "custom": "/custom/*.js" }, "uni": true, "theme": { "primary-color": "#ec2c34" }, "lessVariables": "/variables.module.less" } }.conf.main.mount));
      return new promise((resolve2) => resolve2({ app }));
    }
  };
};
const name = "vue-scaff-uni";
const appid = "__UNI__B522EC0";
const description = "";
...
const pages = [
  {
    ...
    style: {
      navigationBarTitleText: "vue-scaff-uni"
    }
  }
];
const globalStyle = {
...
const _sfc_main = {
  mixins: [
    {
      computed: mapState(registry_mixin.map)
    }
  ],
  data() {
    return {
...
var MiniProgramPage = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__file", "/Users/joenix/Github/@scaff-v3-template/vue-scaff-v3-uni/src/pages/start/index.vue"]]);
var __glob_2_24 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  "default": MiniProgramPage
}, Symbol.toStringTag, { value: "Module" }));
var registry = (context2) => {
  const registry2 = foreach(context2, (pkg, path) => {
    const key = matcher(path);
    const value = def$1(pkg);
    if ({ "conf": { "main": { "app": "App.vue", "mount": "#app" }, "mock": false, "apollo": false, "registry": { "host": true, "api": true, "route": true, "store": true, "mixin": true }, "extract": { "util": "/utils/*.js", "filter": "/filters/*.js", "directive": "/directives/*.js", "route": "/pages/**/route.js", "store": "/pages/**/store.js", "component": "/components/**/*.vue", "style": "/styles/*.less", "i18n": "/i18n/*.js", "md": "/pages/**/*.md", "custom": "/custom/*.js" }, "uni": true, "theme": { "primary-color": "#ec2c34" }, "lessVariables": "/variables.module.less" } }.conf.registry[key] === true) {
      return { key, value };
    }
  });
  return bless(registry2);
};
const def = (dependency) => dependency.default || dependency;
var catcher = (context2, callback) => {
  const modules2 = {};
  for (const path in context2) {
    const depend = def(context2[path]);
    const result = callback(depend, path);
    const { name: name2, pkg } = result || {};
    if (name2) {
      modules2[name2] = pkg;
    }
  }
...
const { extract } = { "conf": { "main": { "app": "App.vue", "mount": "#app" }, "mock": false, "apollo": false, "registry": { "host": true, "api": true, "route": true, "store": true, "mixin": true }, "extract": { "util": "/utils/*.js", "filter": "/filters/*.js", "directive": "/directives/*.js", "route": "/pages/**/route.js", "store": "/pages/**/store.js", "component": "/components/**/*.vue", "style": "/styles/*.less", "i18n": "/i18n/*.js", "md": "/pages/**/*.md", "custom": "/custom/*.js" }, "uni": true, "theme": { "primary-color": "#ec2c34" }, "lessVariables": "/variables.module.less" } }.conf;
var resource = (context2) => {
  return foreach(extract, (observe, key) => {
    const namespace = suffix(observe);
    const exp = translate$1(observe);
    if (namespace) {
      return catcher(context2, (pkg, path) => {
        path = path.replace(/^\/src/, "");
        if (exp.exec(path)) {
          const name2 = matcher(path, exp);
          return { name: name2, pkg };
        }
      });
    }
  });
};
var trigger = { registry, resource };
const context = {
  registry: { "/src/registry/api.js": registry_api.__glob_2_13, "/src/registry/host.js": registry_host.__glob_2_14, "/src/registry/mixin.js": registry_mixin.__glob_2_15, "/src/registry/route.js": registry_route.__glob_2_16, "/src/registry/store.js": registry_store.__glob_2_17 },
  resource: { "/src/App.vue": __glob_2_0, "/src/main.js": __glob_2_1, "/src/manifest.json": __glob_2_2, "/src/pages.json": __glob_2_3, "/src/scaff.ts": scaff.__glob_2_4, "/src/variables.module.less": __glob_2_5, "/src/variables.module.scss": __glob_2_6, "/src/assets/vue-scaff-fox.png": common_assets.__glob_2_7, "/src/custom/plus.js": custom_plus.__glob_2_8, "/src/directives/move.js": directives_move.__glob_2_9, "/src/filters/cash.js": filters_cash.__glob_2_10, "/src/i18n/en-US.js": i18n_enUS.__glob_2_11, "/src/i18n/zh-CN.js": i18n_zhCN.__glob_2_12, "/src/registry/api.js": registry_api.__glob_2_13, "/src/registry/host.js": registry_host.__glob_2_14, "/src/registry/mixin.js": registry_mixin.__glob_2_15, "/src/registry/route.js": registry_route.__glob_2_16, "/src/registry/store.js": registry_store.__glob_2_17, "/src/styles/index.less": __glob_2_18, "/src/styles/sample.less": __glob_2_19, "/src/tools/hello.js": tools_hello.__glob_2_20, "/src/utils/empty.js": utils_empty.__glob_2_21, "/src/utils/foreach.js": utils_foreach.__glob_2_22, "/src/utils/uri.js": utils_uri.__glob_2_23, "/src/pages/start/index.vue": __glob_2_24, "/src/pages/start/store.js": pages_start_store.__glob_2_25 }
};
var context$1 = {
  registry: trigger.registry(context.registry),
  resource: trigger.resource(context.resource)
};
var Scaff = Scaff$1(context$1);
function createApp(app) {
  try {
    app = new Scaff(scaff.Main).result.app;
  } catch (e2) {
    console.log(e2);
  }
  return { app };
